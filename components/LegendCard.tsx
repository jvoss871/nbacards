...
export function LegendCard({ legend, prestigeNum, className = '' }: {
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="rounded-xl overflow-hidden bg-gradient-to-b from-[#0c1a35] via-[#0e1e3a] to-[#050d1a] shadow-2xl shadow-black/50 border border-amber-500/20 flex flex-col">
        ...
        <div className="relative aspect-[5/7]">
          ...
          <div className="absolute top-0 inset-x-0 z-20 bg-black/60 flex items-center justify-between px-2 py-1.5">
            <span className="text-[7px] font-black uppercase tracking-[0.25em] text-amber-400">P·{ROMAN[prestigeNum - 1] ?? prestigeNum}</span>
            <span className="text-[7px] font-black text-white uppercase tracking-wider">{legend.position}</span>
          </div>
          ...
          <div className="absolute bottom-0 inset-x-0 px-2.5 pb-2.5 z-20">
            <div className="text-amber-200 font-black text-[9px] uppercase tracking-[0.12em] leading-tight drop-shadow-lg">
              {legend.name}
            </div>
            <div className="text-amber-400 text-[7px] mt-0.5 uppercase tracking-wider font-bold">{legend.era}</div>
            <div className="text-amber-300/70 text-[6px] mt-0.5 leading-tight line-clamp-1">{legend.bio}</div>
          </div>

        </div>

        {/* Graded label strip */}
        <div className="bg-[#0a1628] px-3 py-1.5 flex items-center justify-between">
          <span className="text-[6px] font-black uppercase tracking-[0.2em] text-amber-400/70">CardPicks</span>
          <span className="text-[7px] font-black uppercase tracking-[0.15em] text-amber-300">Legend</span>
          <span className="text-[6px] font-black text-amber-400/70 tabular-nums">
