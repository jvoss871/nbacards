...
import { fetchActivePlayers, BDL_ACTIVE_TEAM_IDS } from '@/lib/balldontlie'
...
// Draft-position fallback — only used for players without a rating_2k yet.
// Once a player has been through the manual tier pass (/admin/players), their
// tier is authoritative and this fallback no longer applies to them.
...
export async function POST() {
  ...
  let allPlayers
  try {
    allPlayers = await fetchActivePlayers(apiKey, undefined, Object.values(BDL_ACTIVE_TEAM_IDS))
  } catch (e) {
  ...
  // ── 5. Upsert ─────────────────────────────────────────────────
  // Batched rather than one await per player (~500 sequential round trips was
  // taking several minutes). Rows are grouped by shape first — PostgREST's bulk
  // upsert/insert builds one SQL statement per call, so every row in a given
  // call must carry the same set of keys.
  const updateRowsWithImage: Record<string, unknown>[] = []
  const updateRowsNoImage: Record<string, unknown>[] = []
  ...
  for (const player of activePlayers) {
    ...
    // Players already rated (via the manual tier pass) keep their tier.
    ...
    if (existingRow) {
      const base = { ...record, id: existingRow.id }
      // Backfill image_url only if the row currently has none
      if (!existingRow.image_url && imageUrl) {
        updateRowsWithImage.push({ ...base, image_url: imageUrl })
      } else {
        updateRowsNoImage.push(base)
      }
    } else {
      insertRows.push({ ...record, image_url: imageUrl, tier_locked: false })
    }
  }

  const BATCH_SIZE = 250
  async function batched(rows: Record<string, unknown>[], fn: (chunk: Record<string, unknown>[]) => PromiseLike<unknown>) {
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      await fn(rows.slice(i, i + BATCH_SIZE))
    }
  }

  await batched(updateRowsWithImage, chunk => sb.from('players').upsert(chunk, { onConflict: 'id' }))
  await batched(updateRowsNoImage,   chunk => sb.from('players').upsert(chunk, { onConflict: 'id' }))
  await batched(insertRows,          chunk => sb.from('players').insert(chunk))

  const updated  = updateRowsWithImage.length + updateRowsNoImage.length
